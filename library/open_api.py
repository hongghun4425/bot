...
class open_api(QAxWidget):
    def __init__(self):
        ...
        if not self.sf.is_simul_table_exist(self.db_name, "setting_data"):
            ...
            logger.debug("setting_data table 존재한다!!!")

        # 만약에 t_sector 테이블이 존재하지 않으면 구축 하는 로직
        if not self.sf.is_simul_table_exist("daily_craw", "t_sector"):
            self.init_db_t_sector()
        else:
            logger.debug("t_sector table 존재한다!!!")
    ...
    def _receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        ...
        if rqname == "opt10081_req" and self.py_gubun == "trader":
            ...
            # logger.debug("opt20006_req!!!")
            # logger.debug("Get an de_deposit!!!")
            self.collector_opt20006(rqname, trcode)
        # except Exception as e:
        else:
    ...
    def init_db_setting_data(self):
        ...
        #  추가하면 여기에도 추가해야함
        df_setting_data_temp = {'loan_money': [],
                                'limit_money': [],
                                'invest_unit': [],
                                'max_invest_unit': [],
                                'min_invest_unit': [],
        ...
        # t_sector 를 초기화 하는 함수
    def init_db_t_sector(self):
            logger.debug("init_db_t_sector !! ")

            #  추가하면 여기에도 추가해야함
            df_t_sector_temp = {'date': [], 'check_item': [], 'code': [], 'code_name': [], 'd1_diff_rate': [],'d2_diff_rate': [],'d3_diff_rate': [],
                                'd5_diff_rate': [],'d10_diff_rate': [],'d20_diff_rate': [],
            ...
            df_t_sector = DataFrame(df_t_sector_temp,
                                    columns=['date', 'check_item', 'code', 'code_name', 'd1_diff_rate','d2_diff_rate','d3_diff_rate', 'close', 'open',
                                             'd5_diff_rate','d10_diff_rate','d20_diff_rate','high', 'low', 'volume',
            ...
            # 자료형
            df_t_sector.loc[0, 'date'] = str(0)
            df_t_sector.loc[0, 'check_item'] = int(0)
            df_t_sector.loc[0, 'code'] = str(0)
            df_t_sector.loc[0, 'code_name'] = str(0)
            df_t_sector.loc[0, 'd1_diff_rate'] = int(0)
            df_t_sector.loc[0, 'd2_diff_rate'] = int(0)
            df_t_sector.loc[0, 'd3_diff_rate'] = int(0)
            df_t_sector.loc[0, 'd5_diff_rate'] = int(0)
            df_t_sector.loc[0, 'd10_diff_rate'] = int(0)
            df_t_sector.loc[0, 'd20_diff_rate'] = int(0)

            df_t_sector.loc[0, 'close'] = int(0)
            df_t_sector.loc[0, 'open'] = int(0)
            df_t_sector.loc[0, 'high'] = int(0)
            df_t_sector.loc[0, 'low'] = int(0)
            df_t_sector.loc[0, 'volume'] = int(0)

            df_t_sector.loc[0, 'clo5'] = int(0)
            df_t_sector.loc[0, 'clo10'] = int(0)
            df_t_sector.loc[0, 'clo20'] = int(0)
            df_t_sector.loc[0, 'clo40'] = int(0)
            df_t_sector.loc[0, 'clo60'] = int(0)
            df_t_sector.loc[0, 'clo80'] = int(0)
            df_t_sector.loc[0, 'clo100'] = int(0)
            df_t_sector.loc[0, 'clo120'] = int(0)

            df_t_sector.loc[0, 'clo5_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo10_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo20_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo40_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo60_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo80_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo100_diff_rate'] = int(0)
            df_t_sector.loc[0, 'clo120_diff_rate'] = int(0)

            df_t_sector.loc[0, 'yes_clo5'] = int(0)
            df_t_sector.loc[0, 'yes_clo10'] = int(0)
            df_t_sector.loc[0, 'yes_clo20'] = int(0)
            df_t_sector.loc[0, 'yes_clo40'] = int(0)
            df_t_sector.loc[0, 'yes_clo60'] = int(0)
            df_t_sector.loc[0, 'yes_clo80'] = int(0)
            df_t_sector.loc[0, 'yes_clo100'] = int(0)
            df_t_sector.loc[0, 'yes_clo120'] = int(0)

            df_t_sector.loc[0, 'vol5'] = int(0)
            df_t_sector.loc[0, 'vol10'] = int(0)
            df_t_sector.loc[0, 'vol20'] = int(0)
            df_t_sector.loc[0, 'vol40'] = int(0)
            df_t_sector.loc[0, 'vol60'] = int(0)
            df_t_sector.loc[0, 'vol80'] = int(0)
            df_t_sector.loc[0, 'vol100'] = int(0)
            df_t_sector.loc[0, 'vol120'] = int(0)

            df_t_sector.to_sql('t_sector', self.engine_daily_craw, if_exists='replace')

            sql = "delete from t_sector"
            self.engine_daily_craw.execute(sql)
    ...
    def get_total_data_min(self, code, code_name, start):
        ...
        return df
        # get_sector_data : 업종별 1985년 이후 특정 날짜까지의 일별 주가 데이터를 모두 가져오는 함수
        # code : 업종코드 = 001:종합(KOSPI), 002:대형주, 003:중형주, 004:소형주 101:종합(KOSDAQ), 201:KOSPI200, 302:KOSTAR, 701: KRX100
        # date : 기준일자. (ex. '20200424') => 20200424 일자 까지의 모든 open, high, low, close, volume 데이터 출력

    def get_sector_data(self, code, date):
        logger.debug("get_sector_data 함수에 들어왔다!")
        ...
        self.comm_rq_data("opt20006_req", "opt20006", 0, "0101")

        # 만약에 종목 테이블이 없으면 600일 한번만 가져오는게 아니고 몇 천일이던 싹다 가져오는거다.
        if not self.is_craw_sector_exist(code):
            while self.remained_data == True:
                time.sleep(TR_REQ_TIME_INTERVAL)
                self.set_input_value("업종코드", code)
                self.set_input_value("기준일자", date)
                self.comm_rq_data("opt20006_req", "opt20006", 2, "0101")
    ...
    # except Exception as e:
    #     logger.critical(e)
    # t_sector 에 업종 데이타가 존재 여부 확인 함수
    def is_craw_sector_exist(self, code):
        # #jackbot("******************************** is_craw_table_exist !!")
        sql = "select 1 from t_sector where code = '%s'"
        rows = self.engine_daily_craw.execute(sql % code).fetchall()
        if rows:
            return True
        else:
            logger.debug(str(code) + " 업종 데이타가 t_sector table 에 없다. 새로 생성! ", )
            return False
    ...
    def get_craw_db_last_min(self, code_name):
        ...
        # 신생
            ...
            return str(0)
    # t_sector 특정 업종의 데이터에서 마지막으로 콜렉팅한 date를 가져오는 함수
    def get_daily_sector_craw_db_last_date(self, code):
        sql = "SELECT date from t_sector where code = '" + code + "' order by date desc limit 1"
    ...
    # except Exception as e:
    #     logger.critical(e)
    def collector_opt20006(self, rqname, trcode):
        # 몇 개의 row를 읽어 왔는지 담는 변수
        ohlcv_cnt = self._get_repeat_cnt(trcode, rqname)
        for i in range(ohlcv_cnt):
            date = self._get_comm_data(trcode, rqname, i, "일자")
...
#################################### 3.조건검색 부분 #########################################################
    ...
    def condition_tr_slot(self, sScrNo, strCodeList, strConditionName, nIndex, nNext):
        ...
        condition_df.to_sql('stock_condition', self.engine_daily_buy_list, if_exists='replace', dtype=dtypes)
        self.condition_event_loop.exit()