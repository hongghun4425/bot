...
class collector_api():
    ...
    def code_update_check(self):
        ...
        sql = "select code_update,jango_data_db_check, possessed_item, today_profit, final_chegyul_check, db_to_buy_list,today_buy_list, daily_crawler , min_crawler, daily_buy_list, daily_sector_crawler from setting_data limit 1"
        ...
        if rows[0][10] != self.open_api.today:
    ...
    def get_code_list(self):
        ...
        # sector all (업종별 종합지수)
        df_sector_all_temp = {'id': [], 'code': [], 'code_name': [], 'check_item': [], 'check_daily_sector_crawler': []}
    ...
    def set_daily_sector_crawler_table(self, code, code_name):
        ...
        df_temp = DataFrame(df,
                            columns=['date', 'check_item', 'code', 'code_name', 'd1_diff_rate', 'd2_diff_rate',
                                     'd3_diff_rate','d5_diff_rate','d10_diff_rate','d20_diff_rate'
                                      ,'close', 'open', 'high',
                                     'low',
        ...
        df_temp['d2_diff_rate'] = round(
            (df_temp['close'] - df_temp['close'].shift(2)) / df_temp['close'].shift(2) * 100, 2)
        df_temp['d3_diff_rate'] = round(
            (df_temp['close'] - df_temp['close'].shift(3)) / df_temp['close'].shift(3) * 100, 2)
        df_temp['d5_diff_rate'] = round(
            (df_temp['close'] - df_temp['close'].shift(5)) / df_temp['close'].shift(5) * 100, 2)
        df_temp['d10_diff_rate'] = round(
            (df_temp['close'] - df_temp['close'].shift(10)) / df_temp['close'].shift(10) * 100, 2)
        df_temp['d20_diff_rate'] = round(
            (df_temp['close'] - df_temp['close'].shift(20)) / df_temp['close'].shift(20) * 100, 2)
